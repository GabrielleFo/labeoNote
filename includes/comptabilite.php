...
function frais_display_export_table() {
    ...
    if ($results) {
        ...
        echo '<thead><tr><th>Date</th><th>Type de frais</th><th>Montant</th><th>Description</th><th>Utilisateur</th></tr></thead>';
        echo '<tbody>';
...
function frais_export_frais_action() {
    ...
    if ($results) {
        ...
        fputcsv($output, array('Date', 'Type de frais', 'Montant', 'Description'));

        foreach ($results as $row) {
            $user = get_userdata($row['user_id']);
            fputcsv($output, array($row['date'], $row['type'], $row['montant'], $row['description'], $user->display_name));
        }
        fclose($output);
        exit;
    } 
